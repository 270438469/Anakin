...
class CryptoPriceTracker:
    ...
    async def fetch_polymarket_events(self):
        ...
        url = "https://gamma-api.polymarket.com/events/pagination?limit=10000&active=true&archived=false&tag_slug=crypto-prices&closed=false&order=volume24hr&ascending=false&offset=0"
        url = "https://gamma-api.polymarket.com/events/pagination?limit=10000&active=true&archived=false&tag_slug=crypto-prices&closed=false&order=endDate&ascending=true&offset=0"
    ...
    async def process_events(self, data, session):
        """Process events and filter for up-or-down-15m series, collect all markets"""
        ...
        for event in events:
            # Check for 15m series
            has_15m_series = False
            series = event.get("series", [])
            for serie in series:
                if "up-or-down-15m" in serie.get("slug", ""):
                    has_15m_series = True
                    break
            
            # Collect markets only if event has 15m series
            if has_15m_series:
                ...
                for market in markets:
                    ...
                    try:
                        ...
                        # Only include current 15m market (within ~15min window)
                        if 0 < seconds_remaining <= 900:  # 15 minutes = 900 seconds
                            markets_found += 1
                            ...
                            if symbol:
                                ...
                                # Store market token for order book
        ...
        print(f"Found {markets_found} markets within 15min")
    ...
    async def get_trigger_price(self, session, symbol, start_date):
        ...
        try:
            ...
            url = f"https://polymarket.com/api/crypto/crypto-price?symbol={base_symbol}&eventStartTime={start_date}&variant=fifteen"
    ...
    def extract_symbol_from_resolution_source(self, resolution_source):
        """Extract crypto symbol from resolution source URL and normalize to *USDT*"""
        if not resolution_source:
            return None

        try:
            # ========================
            # 1. BINANCE 格式  BTC_USDT / BTCUSDT
            # ========================
            # Match BTC_USDT
            match = re.search(r'/([A-Za-z]+)_USDT', resolution_source)
            if match:
                return match.group(1).upper() + "USDT"

            # Match BTCUSDT
            match = re.search(r'/([A-Za-z]+)USDT', resolution_source)
            if match:
                return match.group(1).upper() + "USDT"

            # ========================
            # 2. CHAINLINK 格式 sol-usd
            # ========================
            # Match sol-usd → SOLUSDT
            match = re.search(r'/([A-Za-z]+)-usd', resolution_source, flags=re.IGNORECASE)
            if match:
                return match.group(1).upper() + "USDT"
    ...
    def calculate_time_to_end(self, end_date_str):
        """Calculate time remaining until market end, including seconds"""
        if not end_date_str:
            return "No end date"
